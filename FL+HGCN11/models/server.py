...
from models.for_GFL_and_HGCNFL import generate_adj, normalize_graph_adj
...
class Server_Net(nn.Module):
    ...
    def hgcn_aggregate_weights_2(self, args, selected_clients):
        total_size = 0
        Ws, dWs, grads = [], [], []
        train_size = []
        for client in selected_clients:
        ...
        for client in selected_clients:
            ...
            Ws.append(W)
            train_size.append(client.train_size / total_size)
        client_weight = graph_parameter(Ws, args, train_size)
...
def graph_dic(models_dic, args, gc_model, total_size, train_size, server_weight):
    ...
    aggregated_param, avg_metrix = generate_adj(param_metrix, args, gc_model, subgraph_size, train_size, server_weight)
    # A = normalize_graph_adj(A)
